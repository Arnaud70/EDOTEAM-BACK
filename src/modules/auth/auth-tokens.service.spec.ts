  ...
  return {
    ...
    authThrottle: {
      findUnique: jest.fn(async ({ where }: any) => {
        ...
        return (
          throttles.find((t) => t.userId === userId && t.type === type) || null
        );
      }),
      upsert: jest.fn(async ({ where, create, update }: any) => {
        const { userId, type } = where.userId_type;
        const existing = throttles.find(
...
describe('AuthTokensService', () => {
  ...
  it('émet un code à 6 chiffres et le consomme une seule fois', async () => {
    ...
    await expect(
      service.consumeCode(uid, 'EMAIL_VERIFICATION' as any, code),
  ...
  it('un code valide réinitialise le compteur de verrouillage', async () => {
    ...
    await service
      .consumeCode(uid, 'EMAIL_VERIFICATION' as any, wrongOf(code))
      .catch(() => {});
  ...
  it('supprime le code après 5 essais incorrects', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      await expect(
        service.consumeCode(uid, 'PASSWORD_RESET' as any, wrong),
      ).rejects.toThrow(/incorrect/i);
    }
    // code supprimé -> il faut en redemander un
    await expect(
      service.consumeCode(uid, 'PASSWORD_RESET' as any, code),
    ).rejects.toThrow(/nouveau code/i);
  });
  ...
  it('verrouille ~1 h après 8 échecs cumulés (sur plusieurs codes)', async () => {
    ...
    await expect(
      service.consumeCode(uid, 'EMAIL_VERIFICATION' as any, wrongOf(code)),
    ).rejects.toThrow(/réessayez dans/i);

    // verrouillé : même un bon code / une nouvelle demande est refusée
    await expect(
      service.consumeCode(uid, 'EMAIL_VERIFICATION' as any, '123456'),
    ).rejects.toThrow(/réessayez dans/i);
    await expect(
      service.issueCode(uid, 'EMAIL_VERIFICATION' as any),
    ).rejects.toThrow(/réessayez dans/i);
    await expect(
      service.assertNotLocked(uid, 'EMAIL_VERIFICATION' as any),
    ).rejects.toThrow(/réessayez dans/i);
  ...
  it("le verrouillage d'un type n'affecte pas l'autre type", async () => {
    ...
    await expect(
      service.assertNotLocked(uid, 'PASSWORD_RESET' as any),
    ).resolves.toBeUndefined();
  });
  ...
  it('rejette un format de code invalide', async () => {
    ...
    await expect(
      service.consumeCode(uid, 'EMAIL_VERIFICATION' as any, 'abc'),
    ).rejects.toThrow();
    await expect(
      service.consumeCode(uid, 'EMAIL_VERIFICATION' as any, '12345'),
    ).rejects.toThrow();
  });
});
